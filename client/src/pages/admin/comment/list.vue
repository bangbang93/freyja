<template>
  <div class="freyja-comment-list">
    <el-table :data="comments">
      <el-table-column label="作者" prop="publisher.name"></el-table-column>
...
<script>
  export default {
    ...
    methods: {
      async initData() {
        let resp = await this.$fetch.get('/api/admin/comment', {page: this.currentPage})
        this.comments = await resp.json()
      },
      async handleDelete(index, row) {
        let resp = await this.$fetch.del(`/api/admin/comment/${row._id}`)
        if (resp.status === 204) {
          await this.initData()
        }
      }
